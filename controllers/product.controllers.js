...
const saveProductController = async (req, res) => {
    ...
    if (!nombre || !descripcion || !codigo || !foto || !precio || !stock) return res.json({ error: 'datos insuficientes' })
    res.json(await Products.saveProduct({ timestamp, nombre, descripcion, codigo, foto, precio, stock }));
}
const updateProductController = async (req, res) => {
    const { id } = req.params;
    if (isNaN(+id)) return res.json({ error: '1-El parametro debe ser un numero' });
    const model = ['nombre', 'descripcion', 'codigo', 'foto', 'precio', 'stock'];//el modelos de las propiedades de un producto
